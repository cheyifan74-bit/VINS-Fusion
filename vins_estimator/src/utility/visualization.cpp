...
void pubOdometry(const Estimator &estimator, const std_msgs::Header &header)
{
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
    {
        ...
        // debug state with named fields
        vins::VinsDebugState debug_msg;
        debug_msg.header = header;
        debug_msg.position.x = estimator.Ps[WINDOW_SIZE].x();
        debug_msg.position.y = estimator.Ps[WINDOW_SIZE].y();
        debug_msg.position.z = estimator.Ps[WINDOW_SIZE].z();
        debug_msg.velocity.x = estimator.Vs[WINDOW_SIZE].x();
        debug_msg.velocity.y = estimator.Vs[WINDOW_SIZE].y();
        debug_msg.velocity.z = estimator.Vs[WINDOW_SIZE].z();
        debug_msg.orientation.w = tmp_Q.w();
        debug_msg.orientation.x = tmp_Q.x();
        debug_msg.orientation.y = tmp_Q.y();
        debug_msg.orientation.z = tmp_Q.z();
        debug_msg.ba.x = estimator.Bas[WINDOW_SIZE].x();
        debug_msg.ba.y = estimator.Bas[WINDOW_SIZE].y();
        debug_msg.ba.z = estimator.Bas[WINDOW_SIZE].z();
        debug_msg.bg.x = estimator.Bgs[WINDOW_SIZE].x();
        debug_msg.bg.y = estimator.Bgs[WINDOW_SIZE].y();
        debug_msg.bg.z = estimator.Bgs[WINDOW_SIZE].z();
        pub_debug_state.publish(debug_msg);
