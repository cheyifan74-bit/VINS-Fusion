...
public:
                                          ...
                                          const Eigen::Vector3d &Pj, const Eigen::Quaterniond &Qj, const Eigen::Vector3d &Vj, const Eigen::Vector3d &Baj, const Eigen::Vector3d &Bgj)
    {
        ...
        if (zupt_flag)
        {
            // ZUPT: identity constraints, no IMU measurement dependence
            residuals.block<3, 1>(O_P, 0) = Qi.inverse() * (Pj - Pi);
            residuals.block<3, 1>(O_R, 0) = 2 * (Qi.inverse() * Qj).vec();
            residuals.block<3, 1>(O_V, 0) = Qi.inverse() * (Vj - Vi);
        }
        else
        {
    ...
    bool zupt_flag = false; // ZUPT: when true, use identity constraints instead of IMU residual
