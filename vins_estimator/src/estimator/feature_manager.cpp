...
void FeatureManager::setDepth(const VectorXd &x)
{
    ...
    for (auto &it_per_id : feature)
    {
        ...
        double old_depth = it_per_id.estimated_depth;
...
void FeatureManager::triangulate(int frameCnt, Vector3d Ps[], Matrix3d Rs[], Vector3d tic[], Matrix3d ric[])
{
    for (auto &it_per_id : feature)
    {
        ...
        if (STEREO && it_per_id.feature_per_frame[0].is_stereo)
        {
            ...
            if (depth > 0)
            {
                // Quality: only accept depth 0.5m~40m (OAK-D baseline 7.5cm)
                if (depth > 0.5 && depth < 40.0)
                    it_per_id.estimated_depth = depth;
                // else: skip, will retry when more observations available
            }
            /*
        ...
        // Quality: only accept depth 0.5m~40m
        if (svd_method > 0.5 && svd_method < 40.0)
         ...
         it != feature.end(); it = it_next)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if (dep_j > 0.5 && dep_j < 40.0)
                    it->estimated_depth = dep_j;
                else
                {
                    // depth out of range after marginalization, discard point
                    feature.erase(it);
                    continue;
                }
            }
